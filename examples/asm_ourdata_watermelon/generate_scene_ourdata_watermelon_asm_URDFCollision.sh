...
# Use active URDF collision meshes directly. ASM_COLLISION_MESH_SCALE=1.0 means
# original URDF collision mesh size; lower values shrink them around centers.
env -u LD_LIBRARY_PATH python spider/preprocess/prepare_asm_mjcf_watermelon.py \
...
bash examples/asm_ourdata_watermelon/process_ourdata_watermelon_PickSpoonBowlParams.sh

env -u LD_LIBRARY_PATH python spider/preprocess/generate_xml_watermelon.py \
  --dataset-dir example_datasets \
  --dataset-name ourdata \
  --robot-type asm \
