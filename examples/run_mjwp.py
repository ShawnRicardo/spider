...
def _write_reward_breakdown_txt(
    ...
    lines.append("")
    lines.append("解释:")
    lines.append("- 这里的 reward 都是负数形式的 penalty。数值越接近 0，说明误差越小、跟踪越好。")
    lines.append("- 数值越负，说明对应误差越大，优化器越不喜欢这个候选控制序列。")
    lines.append("- qpos_rew: 机器人关节角 + 有效物体位姿的加权跟踪项；milk 的空 left_object 已被屏蔽。")
    lines.append("- arm/hand/object 分项是诊断项，单独看哪段机械链没有跟上；它们不是 total_rew 的逐项求和定义。")
    lines.append("- fingertip_rew: 左右手 10 个指尖 site 的世界坐标跟踪项。")
    lines.append("- contact_rew: 接触点 reward；当前 contact_guidance=false 且 contact_rew_scale=0，所以应为 0。")
    lines.append("- total_rew = qpos_rew + qvel_rew + contact_rew + fingertip_rew。")
    lines.append("- rew 是优化器 rollout 后对候选轨迹的总 reward 统计，优化器会选择更大的 rew。")
    lines.append("")
    lines.append("配置权重:")
