...
"""Convert a two-object local reconstructed scene into a SPIDER keypoint dataset.

This _2objs entrypoint keeps SPIDER's existing right_object/left_object slots:
each slot is mapped to one workspace object id such as obj_0 or obj_1.
"""
...
def _single_object_main_unused(
...
def _require_per_object_inputs(
    workspace_path: Path,
    object_id: str,
    inputs: WorkspaceInputs,
) -> None:
    expected_box = workspace_path / "result" / object_id / "box_for_spider.npz"
    expected_mesh_dir = workspace_path / "sam3d" / object_id
    if inputs.box_npz_path != expected_box:
        raise FileNotFoundError(
            "two-object processing requires a per-object bbox file at "
            f"{expected_box}; resolved {inputs.box_npz_path}"
        )
    if not str(inputs.object_ply_path).startswith(str(expected_mesh_dir)):
        raise FileNotFoundError(
            "two-object processing requires a per-object mesh under "
            f"{expected_mesh_dir}; resolved {inputs.object_ply_path}"
        )
    if inputs.object_pose_dir is None:
        raise FileNotFoundError(
            "two-object processing requires per-object pose txt files under "
            f"{workspace_path / 'result' / object_id}"
        )


def _load_object_slot_trajectory(
    *,
    slot: str,
    inputs: WorkspaceInputs,
    cam_c2w: np.ndarray,
    ...
    if inputs.object_pose_dir is None:
        raise FileNotFoundError(f"{slot}_object has no pose directory")

    box_data = np.load(inputs.box_npz_path)
    initial_center_world = box_data["box_center_world"].astype(np.float64)
    centers_world, rotations_world, debug = _load_foundationpose_object_trajectory(
        pose_dir=inputs.object_pose_dir,
        cam_c2w=cam_c2w,
        num_frames=num_frames,
    )
    centers_sim = _rigid_transform_points(
        centers_world,
        rotation=alignment.rotation,
        translation=alignment.translation,
    )
    rotations_sim = np.stack(
        [
            _orthonormalize_rotation(alignment.rotation @ object_rotation)
            for object_rotation in rotations_world
        ],
        axis=0,
    )
    quats_wxyz = np.stack(
        [_rotation_matrix_to_wxyz(object_rotation) for object_rotation in rotations_sim],
        axis=0,
    )
    qpos = np.zeros((num_frames, 7), dtype=np.float32)
    qpos[:, :3] = centers_sim
    qpos[:, 3:] = quats_wxyz.astype(np.float32)
    aligned_initial_center = _rigid_transform_points(
        initial_center_world[None],
        rotation=alignment.rotation,
        translation=alignment.translation,
    )[0]
    return {
        "qpos": qpos,
        "raw_centers_world": centers_world.astype(np.float32),
        "raw_rotations_world": rotations_world.astype(np.float32),
        "centers_sim_before_scene_offset": centers_sim.astype(np.float32),
        "rotations_sim": rotations_sim.astype(np.float32),
        "quats_wxyz": quats_wxyz.astype(np.float32),
        "debug": debug,
        "pose_dir": str(inputs.object_pose_dir),
        "pose_source": "foundationpose_center_pose",
        "initial_center_world": initial_center_world.astype(np.float32),
        "aligned_initial_center_world": aligned_initial_center.astype(np.float32),
        "bbox_size_xyz_m": box_data["box_real_size_xyz_m"].astype(np.float32),
        "scale_factor": float(box_data["scale_factor"]),
        "box_rotation_R": box_data["box_rotation_R"].astype(np.float32),
    }


def _export_object_slot_assets(
    *,
    slot: str,
    inputs: WorkspaceInputs,
    object_name: str,
    ...
    bbox_size_xyz_m: np.ndarray,
    scale_factor: float,
    align_object_mesh_to_bbox: bool,
    object_texture_face_resolution: int,
    object_textured_mesh_max_faces: int,
    object_texture_tile_size: int,
) -> dict:
    object_mesh_dir = Path(
        get_mesh_dir(
            dataset_dir=str(dataset_dir_path),
    ...
    source_has_faces = _source_mesh_has_faces(inputs.object_ply_path)
    visual_mesh, mesh_alignment_debug = _prepare_object_mesh_for_bbox_frame(
        _load_object_mesh(inputs.object_ply_path),
        scale_factor=scale_factor,
        bbox_size_xyz_m=bbox_size_xyz_m,
        align_axes_to_bbox=align_object_mesh_to_bbox,
    )
    visual_obj_path = object_mesh_dir / "visual.obj"
    visual_mesh.export(visual_obj_path)
    ...
    try:
        mesh_vertex_rgb = _extract_mesh_vertex_rgb(visual_mesh)
        if source_has_faces and mesh_vertex_rgb is not None:
            _write_colored_point_ply(
                colored_ply_path,
                points=np.asarray(visual_mesh.vertices, dtype=np.float32),
                colors=mesh_vertex_rgb,
        ...
        logger.warning(
            "Could not export {}_object colored/textured assets from {}: {}",
            slot,
            inputs.object_ply_path,
            exc,
        )
    ...
    logger.info(
        "Saved {}_object visual mesh to {} (source={})",
        slot,
        visual_obj_path,
        inputs.object_ply_path,
    )
    if textured_assets_written:
        logger.info("Saved {}_object textured visual mesh to {}", slot, textured_obj_path)

    return {
        "mesh_dir": object_mesh_dir,
        "mesh_dir_rel": str(object_mesh_dir.relative_to(dataset_dir_path)),
        "visual_obj_path": visual_obj_path,
        "visual_mesh_ply_path": visual_mesh_ply_path,
        "colored_ply_path": colored_ply_path,
        "colored_ply_written": colored_ply_written,
        "colored_ply_num_points": colored_ply_num_points,
        "colored_ply_color_source": colored_ply_color_source,
        "textured_obj_path": textured_obj_path,
        "texture_png_path": texture_png_path,
        "textured_assets_written": textured_assets_written,
        "texture_atlas_size": texture_atlas_size,
        "textured_mesh_face_count": textured_mesh_face_count,
        "source_has_faces": bool(source_has_faces),
        "source_path": str(inputs.object_ply_path),
        "mesh_alignment_debug": mesh_alignment_debug,
    }


def _rel_if_written(path: Path, root: Path, written: bool) -> str | None:
    return str(path.relative_to(root)) if written else None


def _append_slot_debug(debug_items: dict, prefix: str, slot_data: dict, assets: dict) -> None:
    object_debug = slot_data["debug"]
    mesh_debug = assets["mesh_alignment_debug"]
    debug_items.update(
        {
            f"{prefix}_raw_object_centers_world": slot_data["raw_centers_world"],
            f"{prefix}_raw_object_rotations_world": slot_data["raw_rotations_world"],
            f"{prefix}_object_centers_sim": slot_data["qpos"][:, :3],
            f"{prefix}_object_centers_sim_before_scene_offset": slot_data[
                "centers_sim_before_scene_offset"
            ],
            f"{prefix}_object_rotations_sim": slot_data["rotations_sim"],
            f"{prefix}_object_quats_wxyz": slot_data["quats_wxyz"],
            f"{prefix}_object_tracking_valid_mask": object_debug["valid_mask"],
            f"{prefix}_object_pose_source_files": object_debug.get(
                "pose_source_files",
                np.asarray([], dtype=str),
            ),
            f"{prefix}_raw_object_poses_world": object_debug.get(
                "object_poses_world",
                np.zeros((0, 4, 4), dtype=np.float32),
            ),
            f"{prefix}_initial_box_center_world": slot_data["initial_center_world"],
            f"{prefix}_aligned_initial_box_center_world": slot_data[
                "aligned_initial_center_world"
            ],
            f"{prefix}_object_bbox_size_xyz_m": slot_data["bbox_size_xyz_m"],
            f"{prefix}_object_scale_factor": np.array(
                slot_data["scale_factor"], dtype=np.float32
            ),
            f"{prefix}_object_visual_source_path": np.array(assets["source_path"]),
            f"{prefix}_object_colored_ply_path": np.array(
                str(assets["colored_ply_path"])
                if assets["colored_ply_written"]
                else ""
            ),
            f"{prefix}_object_colored_ply_num_points": np.array(
                assets["colored_ply_num_points"], dtype=np.int32
            ),
            f"{prefix}_object_colored_ply_color_source": np.array(
                assets["colored_ply_color_source"]
            ),
            f"{prefix}_object_textured_visual_path": np.array(
                str(assets["textured_obj_path"])
                if assets["textured_assets_written"]
                else ""
            ),
            f"{prefix}_object_texture_png_path": np.array(
                str(assets["texture_png_path"])
                if assets["textured_assets_written"]
                else ""
            ),
            f"{prefix}_mesh_bbox_alignment_applied": np.array(
                mesh_debug["mesh_bbox_alignment_applied"]
            ),
            f"{prefix}_mesh_bbox_alignment_perm": np.asarray(
                mesh_debug["mesh_bbox_alignment_perm"], dtype=np.int32
            ),
            f"{prefix}_mesh_bbox_alignment_rotation": mesh_debug[
                "mesh_bbox_alignment_rotation"
            ].astype(np.float32),
            f"{prefix}_mesh_bbox_alignment_source_extents": mesh_debug[
                "mesh_bbox_alignment_source_extents"
            ].astype(np.float32),
            f"{prefix}_mesh_bbox_alignment_target_extents": mesh_debug[
                "mesh_bbox_alignment_target_extents"
            ].astype(np.float32),
            f"{prefix}_mesh_bbox_alignment_bbox_size": mesh_debug[
                "mesh_bbox_alignment_bbox_size"
            ].astype(np.float32),
            f"{prefix}_mesh_bbox_alignment_max_rel_error": np.array(
                mesh_debug["mesh_bbox_alignment_max_rel_error"], dtype=np.float32
            ),
        }
    )
...
def main(
    workspace: str = "preprocessed/robot",
    dataset_dir: str = f"{spider.ROOT}/../example_datasets",
    dataset_name: str = "ourdata",
    task: str = "robot",
    data_id: int = 0,
    right_object_name: str = "right_obj",
    left_object_name: str = "left_obj",
    right_object_id: str = "obj_1",
    left_object_id: str = "obj_0",
    embodiment_type: str = "bimanual",
    ref_dt: float = 1.0 / 30.0,
    orientation_policy: str = "preserve_box",
    world_to_sim_alignment: str = ALIGNMENT_MODE_NONE,
    ...
    align_object_mesh_to_bbox: bool = True,
    trajectory_interpolation_factor: int = 1,
    scene_offset_xyz: tuple[float, float, float] = (0.0, 0.0, 0.0),
    object_texture_face_resolution: int = 512,
    object_textured_mesh_max_faces: int = 120000,
    object_texture_tile_size: int = 4,
) -> None:
    ...
    dataset_dir_path = Path(dataset_dir).resolve()
    if embodiment_type != "bimanual":
        raise ValueError("two-object ourdata processor only supports bimanual")
    if right_object_id == left_object_id:
        raise ValueError("right_object_id and left_object_id must be different")
    if right_object_name == left_object_name:
        raise ValueError("right_object_name and left_object_name must be different")

    right_inputs = _resolve_workspace_inputs(workspace_path, object_id=right_object_id)
    left_inputs = _resolve_workspace_inputs(workspace_path, object_id=left_object_id)
    _require_per_object_inputs(workspace_path, right_object_id, right_inputs)
    _require_per_object_inputs(workspace_path, left_object_id, left_inputs)

    if right_inputs.world_mocap_path != left_inputs.world_mocap_path:
        raise ValueError("right/left object slots resolved different world_mocap files")
    if right_inputs.camera_npz_path != left_inputs.camera_npz_path:
        raise ValueError("right/left object slots resolved different camera bundles")
    ...
    mocap_data = np.load(right_inputs.world_mocap_path)
    camera_bundle = _load_camera_bundle(right_inputs.camera_npz_path)
    right_verts = mocap_data["right_verts"].astype(np.float32)
    left_verts = mocap_data["left_verts"].astype(np.float32)
    cam_c2w = camera_bundle.cam_c2w

    right_pose_count = len(list(right_inputs.object_pose_dir.glob("*.txt")))
    left_pose_count = len(list(left_inputs.object_pose_dir.glob("*.txt")))
    frame_counts = [
        right_verts.shape[0],
        left_verts.shape[0],
        cam_c2w.shape[0],
    ...
    num_frames = min(frame_counts)
    if num_frames <= 0:
        raise RuntimeError("no overlapping frames found across hands/camera/objects")

    right_verts = right_verts[:num_frames]
    left_verts = left_verts[:num_frames]
    cam_c2w = cam_c2w[:num_frames]
    logger.info(
        "Converting two-object workspace {} -> dataset={} task={} frames={} "
    ...
    right_slot = _load_object_slot_trajectory(
        slot="right",
        inputs=right_inputs,
        cam_c2w=cam_c2w,
        alignment=alignment,
    ...
    left_slot = _load_object_slot_trajectory(
        slot="left",
        inputs=left_inputs,
        cam_c2w=cam_c2w,
        alignment=alignment,
    ...
    qpos_obj_right = right_slot["qpos"].copy()
    qpos_obj_left = left_slot["qpos"].copy()
    ...
    if np.linalg.norm(scene_offset_xyz_arr) > 1e-9:
        ...
        qpos_obj_left[:, :3] += scene_offset_xyz_arr
        logger.info(
            "Applied scene_offset_xyz={} to hands and both object slots",
            scene_offset_xyz_arr.tolist(),
        )
    ...
    right_assets = _export_object_slot_assets(
        slot="right",
        inputs=right_inputs,
        object_name=right_object_name,
        dataset_name=dataset_name,
        dataset_dir_path=dataset_dir_path,
        bbox_size_xyz_m=right_slot["bbox_size_xyz_m"],
        scale_factor=right_slot["scale_factor"],
        align_object_mesh_to_bbox=align_object_mesh_to_bbox,
        object_texture_face_resolution=object_texture_face_resolution,
        object_textured_mesh_max_faces=object_textured_mesh_max_faces,
        object_texture_tile_size=object_texture_tile_size,
    )
    left_assets = _export_object_slot_assets(
        slot="left",
        inputs=left_inputs,
        object_name=left_object_name,
        dataset_name=dataset_name,
        dataset_dir_path=dataset_dir_path,
        bbox_size_xyz_m=left_slot["bbox_size_xyz_m"],
        scale_factor=left_slot["scale_factor"],
        align_object_mesh_to_bbox=align_object_mesh_to_bbox,
        object_texture_face_resolution=object_texture_face_resolution,
        object_textured_mesh_max_faces=object_textured_mesh_max_faces,
        object_texture_tile_size=object_texture_tile_size,
    ...
    task_info = {
        ...
        "data_id": data_id,
        "ref_dt": ref_dt,
        "right_object_mesh_dir": right_assets["mesh_dir_rel"],
        "left_object_mesh_dir": left_assets["mesh_dir_rel"],
        "right_object_convex_dir": None,
        "left_object_convex_dir": None,
        "right_object_colored_ply": _rel_if_written(
            right_assets["colored_ply_path"],
            dataset_dir_path,
            right_assets["colored_ply_written"],
        ),
        "left_object_colored_ply": _rel_if_written(
            left_assets["colored_ply_path"],
            dataset_dir_path,
            left_assets["colored_ply_written"],
        ),
        "right_object_visual_mesh_ply": str(
            right_assets["visual_mesh_ply_path"].relative_to(dataset_dir_path)
        ),
        "left_object_visual_mesh_ply": str(
            left_assets["visual_mesh_ply_path"].relative_to(dataset_dir_path)
        ),
        "right_object_visual_source_has_faces": right_assets["source_has_faces"],
        "left_object_visual_source_has_faces": left_assets["source_has_faces"],
        "right_object_textured_visual": _rel_if_written(
            right_assets["textured_obj_path"],
            dataset_dir_path,
            right_assets["textured_assets_written"],
        ),
        "left_object_textured_visual": _rel_if_written(
            left_assets["textured_obj_path"],
            dataset_dir_path,
            left_assets["textured_assets_written"],
        ),
        "right_object_texture_png": _rel_if_written(
            right_assets["texture_png_path"],
            dataset_dir_path,
            right_assets["textured_assets_written"],
        ),
        "left_object_texture_png": _rel_if_written(
            left_assets["texture_png_path"],
            dataset_dir_path,
            left_assets["textured_assets_written"],
        ),
        "source_workspace": str(workspace_path),
        ...
        "scene_offset_xyz": scene_offset_xyz_arr.tolist(),
        "camera_bundle_path": str(camera_bundle.path),
        "object_pose_source": "two_object_pose_txt",
        "right_object_pose_source": right_slot["pose_source"],
        "left_object_pose_source": left_slot["pose_source"],
        "right_object_pose_dir": right_slot["pose_dir"],
        "left_object_pose_dir": left_slot["pose_dir"],
        "object_scale_factor": right_slot["scale_factor"],
        "right_object_scale_factor": right_slot["scale_factor"],
        "left_object_scale_factor": left_slot["scale_factor"],
        "object_visual_source_path": right_assets["source_path"],
        "right_object_visual_source_path": right_assets["source_path"],
        "left_object_visual_source_path": left_assets["source_path"],
        "object_orientation_policy": orientation_policy,
        "align_object_mesh_to_bbox": align_object_mesh_to_bbox,
        "right_object_bbox_size_xyz_m": right_slot["bbox_size_xyz_m"].tolist(),
        "left_object_bbox_size_xyz_m": left_slot["bbox_size_xyz_m"].tolist(),
        "world_to_sim_alignment_mode": alignment.mode,
        "world_to_sim_rotation_matrix": alignment.rotation.tolist(),
        "world_to_sim_translation_xyz_m": alignment.translation.tolist(),
        "world_to_sim_source_origin_xyz_m": alignment.source_origin.tolist(),
    ...
    debug_items = {
        "raw_right_wrist": raw_qpos_wrist_right,
        "raw_left_wrist": raw_qpos_wrist_left,
        "raw_right_tip_positions": raw_qpos_finger_right[:, :, :3],
        "raw_left_tip_positions": raw_qpos_finger_left[:, :, :3],
        "aligned_right_wrist": qpos_wrist_right,
        "aligned_left_wrist": qpos_wrist_left,
        "right_tip_positions": qpos_finger_right[:, :, :3],
        "left_tip_positions": qpos_finger_left[:, :, :3],
    ...
    _append_slot_debug(debug_items, "right", right_slot, right_assets)
    _append_slot_debug(debug_items, "left", left_slot, left_assets)
    debug_path = output_dir / "conversion_debug.npz"
    np.savez(debug_path, **debug_items)

    logger.info("Saved two-object trajectory keypoints to {}", trajectory_path)
    logger.info("Saved two-object task_info to {}", task_info_path)
    logger.info("Saved two-object conversion debug data to {}", debug_path)
