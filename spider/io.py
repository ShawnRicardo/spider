...
def load_data(
    ...
    # interpolate to match sim_dt IK 参考轨迹插值
...
def format_output_suffix(output_suffix: str) -> str:
    """Normalize a user-facing suffix like 'camlink' -> '_camlink'."""
    normalized = output_suffix.strip()
    if not normalized:
        return ""
    return normalized if normalized.startswith("_") else f"_{normalized}"


def append_output_suffix(filename: str, output_suffix: str) -> str:
    """Insert a normalized suffix before the filename extension."""
    suffix = format_output_suffix(output_suffix)
    if not suffix:
        return filename
    path = Path(filename)
