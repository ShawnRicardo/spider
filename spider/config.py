...
class Config:
    ...
    arm_joint_rew_scale: float = -1.0
    hand_joint_rew_scale: float = -1.0
    ...
    fingertip_order: list = field(default_factory=list)
    fingertip_site_ids: list = field(default_factory=list)
    ...
    right_grasp_ctrl_ids: list = field(default_factory=list)
    left_grasp_ctrl_ids: list = field(default_factory=list)
    right_grasp_ctrl_bias: list = field(default_factory=list)
    left_grasp_ctrl_bias: list = field(default_factory=list)
    right_palm_site_id: int = -1
    left_palm_site_id: int = -1
    contact_len: int = 0
...
def build_fingertip_site_ids(
    mj_model: mujoco.MjModel, embodiment_type: str
) -> tuple[list[tuple[str, str]], list[int]]:
    """Resolve hand fingertip tracking sites used by the MJWP fingertip reward."""
    fingertip_order: list[tuple[str, str]] = []
    fingers = ["thumb", "index", "middle", "ring", "pinky"]
    if embodiment_type in ["bimanual", "right"]:
        fingertip_order.extend(("right", finger) for finger in fingers)
    if embodiment_type in ["bimanual", "left"]:
        fingertip_order.extend(("left", finger) for finger in fingers)

    site_ids: list[int] = []
    missing: list[str] = []
    for side, finger in fingertip_order:
        preferred_names = [
        ...
        for name in preferred_names:
            ...
            if sid != -1:
                break
        if sid == -1:
            for candidate_sid in range(mj_model.nsite):
                name = mujoco.mj_id2name(
                    mj_model, mujoco.mjtObj.mjOBJ_SITE, candidate_sid
                )
                if name is None:
                    continue
                name_l = name.lower()
                if (
                    ...
                    and "hand" in name_l
                    and side in name_l
                    and finger in name_l
                    and "tip" in name_l
                ):
                    sid = int(candidate_sid)
                    break
        if sid == -1:
            missing.append(f"{side}_{finger}")
        else:
            site_ids.append(int(sid))

    if missing:
        loguru.logger.warning(
            "Missing fingertip tracking sites for reward: {}", missing
        )
    return fingertip_order, site_ids
...
def resolve_grasp_bias_ctrl_indices(
    model: mujoco.MjModel,
) -> tuple[list[int], list[float], list[int], list[float]]:
    right_ids: list[int] = []
    right_bias: list[float] = []
    left_ids: list[int] = []
    left_bias: list[float] = []

    for aid in range(model.nu):
        actuator_name = mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_ACTUATOR, aid)
        if actuator_name is None or not actuator_name.endswith("_position"):
            continue
        match = re.match(
            r"(?P<side>right|left)_finger(?P<finger>\d+)_joint(?P<joint>\d+)_position",
            actuator_name,
        )
        if match is None:
            continue
        ...
        finger_idx = int(match.group("finger"))
        joint_idx = int(match.group("joint"))
        if joint_idx == 2:
            continue

        bias = 0.10 if finger_idx == 1 else 0.08
        if side == "right":
            right_ids.append(int(aid))
            right_bias.append(bias)
        else:
            left_ids.append(int(aid))
            left_bias.append(bias)

    return right_ids, right_bias, left_ids, left_bias


def resolve_named_site_id(model: mujoco.MjModel, name: str) -> int:
...
def get_noise_scale(config: Config) -> torch.Tensor:
    ...
    if config.robot_type == "asm" and config.embodiment_type in [
        ...
        noise_scale[:, :, :robot_nu] *= config.joint_noise_scale
...
def process_config(config: Config):
    ...
    # read task info before deriving time-step counts so dataset ref_dt is used
    ...
    if config.simulator == "mjwp":
        ...
        if config.contact_guidance:
            ...
            ) = resolve_grasp_bias_ctrl_indices(model)
            config.contact_order, config.hand_contact_site_ids = (
                build_hand_contact_site_ids(model, config.embodiment_type)
            )
            config.right_palm_site_id = resolve_named_site_id(model, "right_palm")
            config.left_palm_site_id = resolve_named_site_id(model, "left_palm")
            config.right_contact_indices = [
                idx
                for idx, (side, finger) in enumerate(config.contact_order)
                if (side == "right") and (finger in ["thumb", "index", "middle"])
            ]
            config.left_contact_indices = [
                idx
                for idx, (side, finger) in enumerate(config.contact_order)
                if side == "left" and (finger in ["thumb", "index", "middle"])
            ]
        if config.fingertip_rew_scale > 0.0:
            config.fingertip_order, config.fingertip_site_ids = (
                build_fingertip_site_ids(model, config.embodiment_type)
            )
            if not config.fingertip_site_ids:
                raise ValueError(
                    "No fingertip tracking sites resolved while fingertip_rew_scale > 0.0"
                )
            loguru.logger.info(
                "Resolved {} fingertip tracking sites for MJWP reward.",
                len(config.fingertip_site_ids),
            )
