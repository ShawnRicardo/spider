...
def setup_viewer(config: Config, mj_model: mujoco.MjModel, mj_data: mujoco.MjData):
    ...
    if use_viser:
        viser_viewer.init_viser(
            app_name="spider",
            host=getattr(config, "viser_host", None),
            port=getattr(config, "viser_port", None),
        )
        if config.save_rerun:
...
def _make_render_options(include_helpers: bool) -> mujoco.MjvOption:
    ...
    if include_helpers:
        ...
        # Hide helper collision geoms and ref/trace sites used for debugging.
        options.geomgroup[3] = False
        options.geomgroup[4] = False
