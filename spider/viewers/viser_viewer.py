...
def init_viser(
    ...
    server_kwargs: dict[str, Any] = {"label": app_name}
    if host is not None:
        server_kwargs["host"] = host
    if port is not None:
        server_kwargs["port"] = port
    _STATE.server = viser.ViserServer(**server_kwargs)
...
def build_and_log_scene_from_spec(
    ...
    entity_root: str = "mujoco",
    build_ref: bool = True,
    include_debug_groups: bool = False,
) -> list[tuple[Any, int]]:
    ...
    for body in spec.bodies[1:]:
        ...
        for geom in body.geoms:
            ...
            # Main workflows hide debug groups (>=5). IK browser debugging can
            # opt in so bbox/axes overlays remain visible in Viser.
            try:
                gv = (
                ...
                if gv >= 5 and not include_debug_groups:
                    continue
            except Exception:
                pass
...
def build_and_log_scene(
    ...
    body_entity_and_ids = build_and_log_scene_from_spec(
        ...
        include_debug_groups=include_debug_groups,
    )
