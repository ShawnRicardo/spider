...
asm.urdf to MJCF, preserves the URDF collision meshes as the robot collision
source of truth, and adds the SPIDER-specific sites and position actuators needed
by generate_xml.py, ik.py, and run_mjwp.py.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.7,
        help=(
            "Scale factor for active URDF collision mesh geoms. The visual meshes "
            "are left unchanged; collision meshes are duplicated and scaled around "
            "their compiled mesh bbox centers."
        ),
    )
...
def activate_urdf_collision_meshes(root: ET.Element) -> None:
    """Keep URDF collision meshes as active MuJoCo collision geoms.

    The URDF contains both visual and collision mesh entries. We keep the visual
    meshes non-colliding and make the imported collision meshes explicit,
    named, massless contact geoms so downstream scene generation can refer to
    them without adding hand-written contact geometry.
    """
...
def validate_model(xml_path: Path, variant: str) -> None:
    ...
    for gid in range(model.ngeom):
        ...
        if model.geom_type[gid] != mujoco.mjtGeom.mjGEOM_MESH:
            raise RuntimeError(f"Non-mesh robot collision geom in {xml_path}: {name}")
        if model.geom_contype[gid] == 0 or model.geom_conaffinity[gid] == 0:
            raise RuntimeError(f"Disabled robot collision mesh in {xml_path}: {name}")
        collision_count += 1
    ...
    if collision_count == 0:
        raise RuntimeError(f"No active URDF collision mesh geoms in {xml_path}")
    ...
    print(
        ...
        f"collision_urdf={collision_count} "
...
def main() -> None:
    ...
    wrap_worldbody_in_root(root, ASM_ROOT_YAW_DEG)
