...
def _make_collision_debug_scene_option() -> mujoco.MjvOption:
    opt = mujoco.MjvOption()
    mujoco.mjv_defaultOption(opt)
    for group_idx in range(len(opt.geomgroup)):
        opt.geomgroup[group_idx] = False
    # Keep normal scene context such as table/object/floor, and show ASM
    # collision proxies. Visual robot meshes are hidden so capsules/boxes are
    # not occluded.
    opt.geomgroup[0] = True
...
def _count_collision_debug_geoms(model: mujoco.MjModel) -> dict[str, int]:
    counts: Counter[str] = Counter()
    for gid in range(model.ngeom):
        geom_name = _geom_name(model, gid)
        if not geom_name.startswith("collision_"):
            continue
...
def _set_mocap_targets_from_reference(
    data: mujoco.MjData,
    index_map: dict,
    qpos_ref: np.ndarray,
    frame_idx: int,
    ...
    if qpos_ref.shape[0] == 0:
        return
    frame_idx = int(np.clip(frame_idx, 0, qpos_ref.shape[0] - 1))
    for value in index_map.values():
        mocap_idx = value.get("mocap_idx", -1)
        if mocap_idx == -1:
            continue
        ref_idx = value["qpos_idx"]
        data.mocap_pos[mocap_idx] = qpos_ref[frame_idx, ref_idx, :3]
        data.mocap_quat[mocap_idx] = qpos_ref[frame_idx, ref_idx, 3:]


def _extract_object_qpos_from_full_qpos(
    qpos_trajectory: np.ndarray,
    embodiment_type: str,
    act_scene: bool,
    fallback_qpos_obj_right: np.ndarray,
    fallback_qpos_obj_left: np.ndarray,
    fallback_frame_indices: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    if not act_scene:
        if embodiment_type == "bimanual":
            return qpos_trajectory[:, -14:-7], qpos_trajectory[:, -7:]
        if embodiment_type == "right":
            qpos_obj_left = np.zeros((qpos_trajectory.shape[0], 7), dtype=np.float64)
            qpos_obj_left[:, 3] = 1.0
            return qpos_trajectory[:, -7:], qpos_obj_left
        if embodiment_type == "left":
            qpos_obj_right = np.zeros((qpos_trajectory.shape[0], 7), dtype=np.float64)
            qpos_obj_right[:, 3] = 1.0
            return qpos_obj_right, qpos_trajectory[:, -7:]

    fallback_frame_indices = np.clip(
        fallback_frame_indices, 0, fallback_qpos_obj_right.shape[0] - 1
    )
    return (
        fallback_qpos_obj_right[fallback_frame_indices],
        fallback_qpos_obj_left[fallback_frame_indices],
    )
...
def main(
    ...
    if not enable_collision:
        ...
        loguru.logger.info(
            "IK collision disabled: contact is disabled for both the IK constraint model and the rollout model."
        )
    ...
    if save_video:
        ...
        collision_renderer = mujoco.Renderer(mj_model, height=480, width=720)
        collision_scene_option = _make_collision_debug_scene_option()
        render_d435_video = _has_camera(mj_model_ik, D435_RENDER_CAMERA_NAME)
        if not render_d435_video:
            loguru.logger.warning(
                "D435 IK video requested by save-video path, but camera '{}' was not found in the IK model. Only front videos will be saved.",
                D435_RENDER_CAMERA_NAME,
            )
        loguru.logger.info(
            "IK collision debug video will render collision geoms: {}",
            _count_collision_debug_geoms(mj_model),
        )
    else:
        render_d435_video = False
    ...
    with run_viewer() as gui:
                ...
                if render_d435_video:
                    if use_debug_render:
                        renderer.update_scene(
                            data=mj_data_ik,
                            camera=D435_RENDER_CAMERA_NAME,
                            scene_option=opt,
                        )
                    else:
                        renderer.update_scene(
                    ...
                    d435_images.append(renderer.render())
                collision_renderer.update_scene(
                    data=mj_data,
                    camera="front",
                    scene_option=collision_scene_option,
                )
                collision_images.append(collision_renderer.render())
        ...
        if save_video:
            ...
            if render_d435_video:
                d435_video_path = (
                    f"{file_dir}/visualization_ik_d435"
                    f"{video_debug_suffix}{'_act' if act_scene else ''}.mp4"
                )
                loguru.logger.info(
                    "Encoding IK D435 video with {} frames to {}",
                    len(d435_images),
                    d435_video_path,
                )
                imageio.mimsave(
                ...
                loguru.logger.info("Saved IK D435 video to {}", d435_video_path)
                d435_images = []
            collision_video_path = (
                f"{file_dir}/visualization_ik_collision_geoms"
                f"{video_debug_suffix}{'_act' if act_scene else ''}.mp4"
            )
            loguru.logger.info(
                "Encoding IK collision debug video with {} frames to {}",
                len(collision_images),
                collision_video_path,
            )
            imageio.mimsave(
                collision_video_path,
            ...
            loguru.logger.info(
                "Saved IK collision debug video to {}",
                collision_video_path,
            )
            collision_images = []

        def render_trajectory_kinematic_video(
            qpos_trajectory: np.ndarray,
            qvel_trajectory: np.ndarray,
        ) -> None:
            if not save_video or qpos_trajectory.shape[0] == 0:
                return
            import imageio
            ...
            debug_qpos_obj_right, debug_qpos_obj_left = _extract_object_qpos_from_full_qpos(
                qpos_trajectory,
                embodiment_type,
                act_scene,
                qpos_obj_right,
            ...
            qpos_render_source = mujoco.MjData(mj_model_ik)
            collision_render_source = mujoco.MjData(mj_model)
            kinematic_images = []
            kinematic_d435_images = []
            kinematic_collision_images = []
            for frame_idx in range(qpos_trajectory.shape[0]):
                qpos_render_source.qpos[:] = qpos_trajectory[frame_idx]
                qpos_render_source.qvel[:] = 0.0
                if qvel_trajectory.shape[0] == qpos_trajectory.shape[0]:
                    qpos_render_source.qvel[:] = qvel_trajectory[frame_idx]
                collision_render_source.qpos[:] = qpos_trajectory[frame_idx]
                collision_render_source.qvel[:] = 0.0
                if (
                    qvel_trajectory.shape[0] == qpos_trajectory.shape[0]
                    and qvel_trajectory.shape[1] == collision_render_source.qvel.shape[0]
                ):
                    collision_render_source.qvel[:] = qvel_trajectory[frame_idx]
                _set_mocap_targets_from_reference(
                    mj_data_ik,
                    index_map,
                    qpos_ref,
                    int(ref_frame_indices[frame_idx]),
                )
                _sync_debug_visualization_state(
                    model=mj_model_ik,
                    data=mj_data_ik,
                    qpos_source=qpos_render_source,
                    frame_idx=frame_idx,
                    qpos_obj_right=debug_qpos_obj_right,
                    qpos_obj_left=debug_qpos_obj_left,
                    debug_object_mocap_ids=debug_object_mocap_ids,
                    debug_robot_base_mocap_id=debug_robot_base_mocap_id,
                    robot_base_body_id=robot_base_body_id,
                    debug_head_camera_mocap_id=debug_head_camera_mocap_id,
                ...
                renderer.update_scene(data=mj_data_ik, camera="front", scene_option=opt)
                kinematic_images.append(renderer.render())
                if render_d435_video:
                    renderer.update_scene(
                        data=mj_data_ik,
                        camera=D435_RENDER_CAMERA_NAME,
                    ...
                    kinematic_d435_images.append(renderer.render())
                mujoco.mj_kinematics(mj_model, collision_render_source)
                collision_renderer.update_scene(
                    data=collision_render_source,
                    camera="front",
                    scene_option=collision_scene_option,
                )
                kinematic_collision_images.append(collision_renderer.render())

            video_path = (
                f"{file_dir}/visualization_trajectory_kinematic"
                f"{video_debug_suffix}{'_act' if act_scene else ''}.mp4"
            )
            loguru.logger.info(
                "Encoding trajectory_kinematic video with {} frames to {}",
                len(kinematic_images),
                video_path,
            )
            imageio.mimsave(
                video_path,
            ...
            loguru.logger.info("Saved trajectory_kinematic visualization to {}", video_path)
            if render_d435_video:
                d435_video_path = (
                    f"{file_dir}/visualization_trajectory_kinematic_d435"
                    f"{video_debug_suffix}{'_act' if act_scene else ''}.mp4"
                )
                loguru.logger.info(
                    "Encoding trajectory_kinematic D435 video with {} frames to {}",
                    len(kinematic_d435_images),
                    d435_video_path,
                )
                imageio.mimsave(
                    d435_video_path,
                ...
                loguru.logger.info(
                    "Saved trajectory_kinematic D435 video to {}",
                    d435_video_path,
                )
            collision_video_path = (
                f"{file_dir}/visualization_trajectory_kinematic_collision_geoms"
                f"{video_debug_suffix}{'_act' if act_scene else ''}.mp4"
            )
            loguru.logger.info(
                "Encoding trajectory_kinematic collision debug video with {} frames to {}",
                len(kinematic_collision_images),
                collision_video_path,
            )
            imageio.mimsave(
                collision_video_path,
            ...
            loguru.logger.info(
                "Saved trajectory_kinematic collision debug video to {}",
                collision_video_path,
            )
        ...
        render_trajectory_kinematic_video(qpos_list, qvel_list)
