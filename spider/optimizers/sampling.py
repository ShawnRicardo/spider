...
def _sample_ctrls_impl(
    ...
    # 随机高斯噪声 * 每个控制维度的噪声 scale * 当前 iteration 的全局退火系数
    knot_samples = (
...
def make_rollout_fn(
    ...
    def rollout(
        ...
        给一批控制序列，全部跑一遍仿真，并给每条打分
        """
        # save initial state 保存当前的仿真状态
        ...
        # rollout to get reward 。H 是 Horizon steps，就是 MJWP 每次优化时会看未来多久的轨迹
        # 这里 Horizon 是 1.0，也就是每次优化时会看未来 1.0 秒的轨迹，Horizon 内每隔 sim_dt 就会有一个控制 step，所以 H = Horizon / sim_dt= 1 / 0.01=100 步
        # 所以每次优化时，他会问，从当前时刻开始，未来 100 个仿真小步里，控制怎么选，才能最接近 IK 参考轨迹？
        N, H = ctrls.shape[:2]
        trace_list = []
